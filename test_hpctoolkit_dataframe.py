...
_LOGS = pathlib.Path(os.environ.get('LOGGING_PATH', str(_HERE.joinpath(
    'tests', datetime.datetime.now().strftime('%Y%m%d-%H%M%S')))))
...
class Tests(unittest.TestCase):
    ...
    def test_load_some(self):
        for path in [_HERE.joinpath(_) for _ in [
                'results/profile_20180911-100231_subset_Sod_base_papi_db/experiment.xml',
                'results/tmp_666_db/experiment.xml']]:
    ...
    def test_load(self):
        for path in _HERE.glob('results/*/experiment.xml'):
            with self.subTest(path=path):
                df = HPCtoolkitDataFrame(path=path)
                hot_path = df.hot_path()
                self.assertIsInstance(hot_path, HPCtoolkitDataFrame)
